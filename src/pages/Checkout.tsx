...
import { ShieldCheck, ArrowLeft, CreditCard, CheckCircle2, Ticket, Sparkles, Loader2, Landmark, LocateFixed } from 'lucide-react';
...
export const Checkout: React.FC = () => {
  ...
  // Real-time GPS location for delivery
  ...
  const handleUseCurrentLocation = () => {
    if (!navigator.geolocation) {
      showToast('Trình duyệt của bạn không hỗ trợ GPS/Geolocation.', 'error');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        if (detectedAddress) {
          setShippingForm((prev) => ({ ...prev, address: detectedAddress }));
          setErrors((prev) => ({ ...prev, address: '' }));
          showToast('Đã lấy vị trí GPS và tự động điền địa chỉ giao hàng.', 'success');
        } else {
          showToast('Đã lấy GPS thành công, nhưng chưa đọc được địa chỉ tự động.', 'info');
        }
      },
      (error) => {
        setIsLocating(false);
        if (error.code === error.PERMISSION_DENIED) {
          showToast('Bạn đã từ chối quyền truy cập vị trí. Hãy cho phép GPS để dùng tính năng này.', 'error');
          return;
        }
        if (error.code === error.POSITION_UNAVAILABLE) {
          showToast('Không thể xác định vị trí hiện tại. Hãy thử lại ở nơi có tín hiệu tốt hơn.', 'error');
          return;
        }
        if (error.code === error.TIMEOUT) {
          showToast('Quá thời gian lấy vị trí GPS. Vui lòng thử lại.', 'error');
          return;
        }

        showToast('Có lỗi khi lấy vị trí GPS.', 'error');
      },
      {
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="pt-2 space-y-3">
                    <button
                      ...
                      className="inline-flex items-center gap-2 bg-neutral-950 hover:bg-neutral-800 text-white text-[11px] uppercase tracking-wider font-bold py-2.5 px-4 rounded-none shadow-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      <LocateFixed size={13} />
                      <span>
                        {isLocating
                    ...
                    <p className="text-[10px] text-neutral-500 leading-relaxed">
                      Bấm nút để lấy vị trí thật từ thiết bị và pin điểm giao hàng trên bản đồ. Trình duyệt sẽ xin quyền truy cập GPS.
                    </p>

                    {gpsLocation && (
                      <div className="space-y-2">
                        <div className="text-[10px] text-neutral-600 bg-[#faf9f6] border border-neutral-200 p-2.5">
                          <p>
                            Tọa độ GPS: {gpsLocation.lat.toFixed(6)}, {gpsLocation.lng.toFixed(6)}
                          </p>
                          <p>Độ chính xác ước tính: ±{Math.round(gpsLocation.accuracy)}m</p>
                        </div>

                        <div className="h-64 w-full border border-neutral-200 overflow-hidden">
                          <MapContainer
                          ...
                          >
                            ...
                            <CircleMarker
                              center={[gpsLocation.lat, gpsLocation.lng]}
                              radius={11}
                              pathOptions={{ color: '#111827', fillColor: '#c9a96e', fillOpacity: 0.9, weight: 2 }}
                            >
                              <Popup>
                                Điểm giao hàng GPS<br />
      ...
      <AnimatePresence>
        {isProcessing && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-neutral-950/80 backdrop-blur-sm z-[2000] flex flex-col items-center justify-center text-white p-4"
          >
      ...
      <AnimatePresence>
        {isSuccessModalOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-neutral-950/80 backdrop-blur-sm z-[2000] flex items-center justify-center p-4"
          >
            <motion.div
